...
const Calc = () => {
  ...
  const dayAmountBTN = () => {
    ...
    resultBtn.addEventListener("click", function (event) {
      let firstResult =
        (inputs[0].value * inputs[1].value) / inputs[2].value - dayAmount; //liczy na ile dni pozostalo dawek
      ...
      if (lastResult >= 0 && lastResult % inputs[1].value === 0) {
        ...
        const maxDosesAmount =
          Math.floor(
            (Math.floor(lastResult) + parseInt(inputs[3].value)) /
              parseInt(inputs[3].value)
          ) * parseInt(inputs[3].value); //liczy maksymalna ilosc dawek po dodaniu najmniejszego zarejestrowanego opakowania.
    ...
    <>
      <header>
        ...
        <div className="heading">
          <h1>Pomożemy Ci szybko sprawdzić ile leku pozostało do wydania.</h1>
        </div>
      </header>
      <main className="container">
        <div className="calendar">
          ...
          />
          <button className="day_amount_btn" onClick={dayAmountBTN}>
            Oblicz ile dni upłynęło
          </button>
        </div>
        <div className="day-amount">
          Od wystawienia e-recepty upłynęło:
          <span className="how_many_days"></span>
        </div>
        <div className="add_info unvisible">
          <p>Możesz całkowicie zrealizować receptę!</p>
          <ul>
            <p>Należy jedynie sprawdzić:</p>
            <li>poprawność dawkowania,</li>
            <li>
              poprawnosć danych pacjenta, osoby wystawiającej receptę oraz
              świadczeniodawcy,
            </li>
            <li>
              jeśli przepisany jest antybiotyk: czy nie upłynęło 7 dni od daty
              wystawienia recepty.
            </li>
          </ul>
        </div>
        <div className="for_year  unvisible">
          ...
          <span
            className="tooltip"
            data-text="Pamiętaj! Recepta na narkotyki i psychotropy jest ważna : 30 dni, na preparaty immunologiczne - 120dni, a na antybiotyki - 7dni!"
          />
        ...
        <div className="deny-div unvisible">
          Tej recepty nie możesz już zrealizować.
        </div>
        <div className="more_info unvisible">
          ...
          <span
            className="tooltip"
            data-text="Jeśli na recepcie przypisano insulinę, należy podać całkowitą ilość jednostek w opakowania np. 1500."
          >
            <input type="number" />
          </span>
          <label>
            Ilość tabletek/dawek/ml leku, którą ma przyjmować pacjent na dobę :
          </label>
          <input type="number" />
          <label>Wielkość najmnniejszego zarejestrowanego opakowania :</label>
          <span
            className="tooltip"
            data-text="W przypadku insulin jest to ilość jednostek w pojedyńczej ampułce/penie!"
          >
            <input type="number" />
          </span>
        ...
        <div className="result unvisible">
          <p>
            Do wydania pozostało maksymalnie : <span></span>{" "}
            tabletki/-ek/dawki/-ek/ml leku
          </p>
          <p>
            Teraz sprawdź ile dostępnych opakowań leku/ampułek inusliny mieści
            się w tym zakresie.
          </p>
        </div>
      </main>
      <footer>
        Kalkulator e-recepty jest jedynie narzędziem pomocniczym. Autor strony
        nie ponosi odpowiedzialności za realizację e-recepty niezgodnie z
        aktualnie obowiązującymi przepisami prawa farmaceutycznego.
        Odpowiedzialność ta spoczywa na personelu apteki, który realizuje
        receptę.
        <span> &copy;rx-kalkulator Ewelina Rij || info@rx-kalkulator.pl</span>
      </footer>
    </>
  );
};
